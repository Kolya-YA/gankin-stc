...
class CourseController extends Controller
{
	...
	public function filterOwnerOnly($filterChain)
	{
		...
		{
			$course = Course::model()->findByPk($this->actionParams['id']);
			if (!$course)
				throw new CHttpException(404);
			if (Yii::app()->user->id != $course->school->user_id)
...
// 			),
	...
	public function actionAdmin()
	{
		...
		{
			$school = School::model()->count('user_id = :uid', array('uid' => Yii::app()->user->id));
			if (!$school)
				$this->redirect('/school/settings');
		}
